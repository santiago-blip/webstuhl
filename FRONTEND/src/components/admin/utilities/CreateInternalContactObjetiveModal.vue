...
import { Loading, useQuasar } from 'quasar'
    ...
    const SubmitData = ()=>{
        ...
        let request = null
        if(!data.value.id){
            const url = 'http://127.0.0.1:8000/vacancies/master/internalRelationsObjetives/'
            ...
            .finally(()=>{
                loading.value = false
                data.value = {Internal_relations_objetives_name:""}
    ...
    <q-dialog v-model="showModalNewInternalContact" persistent>
        <q-card>
            <div class="q-pa-md">
                ...
                <q-input
                    outlined
                    v-model="data.Internal_relations_objetives_name"
                    :label="$t('internalContactsObjetives')"
                    lazy-rules
