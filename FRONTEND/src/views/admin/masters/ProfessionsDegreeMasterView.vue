<script setup>
  ...
  import ModalCreateProfessionDegree from '@/components/admin/utilities/CreateProfessionDegreeModal.vue'
  ...
  const $q = useQuasar()
  const columns = [
                        { name: 'profession_degree_name_id',label: 'Id',align: 'left',sortable: true},
                        { name: 'profession_degree_name',label: 'Nombre',align: 'left',sortable: true},
...
const getData = ()=>{
  ...
  // if(!areaStore.getAllAreas.length){}
    axios.get('http://127.0.0.1:8000/vacancies/master/professionsDegree/')
    .then(response=>{
  ...
  const cleanData = ()=>{
    professionData.value = {profession_degree_name:""}
    getData()
  }
  const submittedData = ()=>{
    cleanData()
  ...
  const titleDelete = 'deleteProfessionDegree'
  const nameToDelete = ref('')
  const openDeleteModal = (data)=>{
    nameToDelete.value = data.profession_degree_name
    professionData.value = data
...
<template>
    ...
    <div class="q-pa-md">
      ...
      <template v-slot:loading>
        ...
        <template v-slot:top>
            <span><b>{{$t('professionDegree')}}</b></span>
            <q-space />
            <!-- <q-btn @click="openModalCreateVacancie" color="primary" icon="add" label="Nueva vacante"/> -->
            <q-btn @click="openModalNewProfession" color="primary" icon="add" :label="$t('newProfessionDegree')" no-caps/>
            <div v-if="showModalNewProfession">
              <ModalCreateProfessionDegree :professionData="professionData" :showModalNewProfession="showModalNewProfession" @closeModalProfession="closeModalNewProfession" @submittedData="submittedData"/>
            </div>
        </template>
        <template v-slot:body="props">
          <q-tr :props="props">
            <q-td key="profession_degree_name_id" :props="props">
              {{props.row.id}}
            </q-td>
            <q-td key="profession_degree_name" :props="props">
              {{props.row.profession_degree_name}}
            </q-td>
