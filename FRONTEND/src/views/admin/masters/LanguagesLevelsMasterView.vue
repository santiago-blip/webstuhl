<script setup>
  ...
  import ModalCreateLanguageLevel from '@/components/admin/utilities/CreateLanguageLevelModal.vue'
  const $q = useQuasar()
  const columns = [
                        {name: 'language_id',label: 'id',align: 'left',sortable: true},
                        {name: 'language_level',label: 'Lenguaje',align: 'left',sortable: true},
  ...
  const languageData = ref({language_level:""})
  ...
  const cleanData = ()=>{
    languageData.value = {language_level:""}
    getData()
  }
  const submittedData = ()=>{
    cleanData()
  ...
  const titleDelete = 'deleteLanguageLevel'
  const nameToDelete = ref('')
  const openDeleteModal = (data)=>{
    nameToDelete.value = data.language_level
    languageData.value = data
...
<template>
    ...
    <div class="q-pa-md">
      ...
      >
        <template v-slot:top>
            <span><b>{{$t('languagesLevels')}}</b></span>
            <q-space />
            <!-- <q-btn @click="openModalCreateVacancie" color="primary" icon="add" label="Nueva vacante"/> -->
            <q-btn @click="openModalNewLanguage" color="primary" icon="add" :label="$t('newLanguageLevel')" no-caps/>
            <div v-if="showModalNewLanguage">
              <ModalCreateLanguageLevel :languageData="languageData" :showModalNewLanguage="showModalNewLanguage" @closeModalNewLanguage="closeModalNewLanguage" @submittedData="submittedData"/>
            </div>
        </template>
